...
- **Voucher sản phẩm**: `nhap` → `cho_duyet` → `da_duyet` → `dang_ban` → `tam_ngung` / `ngung_ban`; nhánh `tu_choi`.
- **Đơn hàng**: `cho_thanh_toan` → `da_thanh_toan` → `da_hoan_tien`; nhánh `da_huy`.
- **Voucher code**: `chua_su_dung` → `da_su_dung` / `het_han` / `bi_huy` / `bi_khoa`.
...
- **AC**:
  ...
  - AC5: chỉ cho cập nhật khi `nhap` hoặc `tu_choi`.
...
- **AC**:
  - AC1: gửi duyệt voucher `nhap`/`tu_choi` → `cho_duyet`.
...
- **AC**: AC1 duyệt `cho_duyet` → `da_duyet`; AC2 từ chối → `tu_choi` + lý do; AC3 công bố `da_duyet` → `dang_ban`; AC4 tạm ngưng `dang_ban` → `tam_ngung` + ẩn khỏi danh sách; AC5 chỉ chuyển `dang_ban` khi đang `da_duyet`.
...
- **FLOW-006**: tạo voucher `nhap` (validate giá/thời gian) → gửi duyệt `cho_duyet` → admin duyệt `da_duyet` → công bố `dang_ban`.
