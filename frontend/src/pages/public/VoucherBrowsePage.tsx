...
export function VoucherBrowsePage() {
  ...
  const vouchers = useMemo(() => {
    ...
    if (sort === 'discount') items.sort((a, b) => (1 - Number(b.salePrice) / Number(b.originalPrice)) - (1 - Number(a.salePrice) / Number(a.originalPrice)))
    return items
  }, [query.data, sort])
    ...
    <section style={{ display: 'flex', flexDirection: 'column', gap: 28, maxWidth: 1200, margin: '0 auto' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 12, flex: '0 1 auto', flexWrap: 'wrap' }}>
          <Stat value={filterOptionsQuery.isLoading ? '…' : String(filterOptionsQuery.data?.categories.length ?? 0)} label='Danh mục' />
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 10 }}>
        <label htmlFor='catalogue-sort' style={{ fontFamily: fonts.display, fontWeight: 700, fontSize: 13 }}>Sắp xếp</label>
        <select id='catalogue-sort' value={sort} onChange={(event) => { const next = new URLSearchParams(urlParams); if (event.target.value === 'newest') next.delete('sort'); else next.set('sort', event.target.value); setUrlParams(next) }} style={{ padding: '10px 38px 10px 14px', borderRadius: 999, border: `1px solid ${colors.hairline}`, background: colors.surface, fontFamily: fonts.body }}>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div style={{ minWidth: 92, padding: '14px 16px', border: '1px solid rgba(255,255,255,.16)', borderRadius: radius.lg }}>
