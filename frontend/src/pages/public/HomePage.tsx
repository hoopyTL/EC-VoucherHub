...
export function HomePage() {
  ...
  return (
    <div style={{ maxWidth: 1240, margin: '0 auto', position:'relative' }}>
      <section className='ticket-hero' style={{ position: 'relative', padding:'clamp(34px,7vw,82px) clamp(22px,5vw,64px)', overflow: 'hidden', borderRadius:'18px 64px 18px 64px', background:'radial-gradient(circle at 83% 18%,rgba(255,117,79,.3),transparent 24%),radial-gradient(circle at 72% 76%,rgba(253,190,71,.28),transparent 22%),linear-gradient(125deg,#fffaf5 0%,#f7efe8 54%,#fde4d8 100%)', boxShadow:'0 24px 70px rgba(91,53,33,.10)' }}>
        <div aria-hidden='true' style={{position:'absolute',right:'7%',top:'13%',width:'clamp(150px,24vw,310px)',aspectRatio:'1',border:'1px solid rgba(228,77,38,.28)',borderRadius:'44% 56% 61% 39% / 49% 37% 63% 51%',transform:'rotate(12deg)'}} />
        <div aria-hidden='true' style={{position:'absolute',right:'13%',bottom:'7%',fontFamily:fonts.display,fontSize:'clamp(52px,10vw,132px)',fontWeight:900,color:'rgba(228,77,38,.075)',letterSpacing:'-.08em'}}>ƯU ĐÃI</div>
      ...
      >
        ...
        >
          <span style={{ color: colors.onInkMuted, fontSize: 12, letterSpacing: '.12em', textTransform: 'uppercase' }}>Tuyển chọn hôm nay</span>
          <div>
            <strong style={{ display: 'block', maxWidth: 600, fontFamily: fonts.display, fontSize: 'clamp(32px, 5vw, 52px)', lineHeight: 1.05 }}>
              Săn ưu đãi đến 50%, trải nghiệm nhiều hơn.
            </strong>
            <span style={{ display: 'inline-block', marginTop: 20, color: '#FFB19C', fontWeight: 700 }}>Khám phá ưu đãi nổi bật →</span>
          </div>
        </Link>
        <div
          style={{
        ...
        >
          ...
          <div>
            <strong style={{ display: 'block', fontFamily: fonts.display, fontSize: 28, lineHeight: 1.1 }}>Thanh toán an toàn</strong>
            <p style={{ margin: '12px 0 0', opacity: .86, lineHeight: 1.6 }}>VNPay cho nội địa và thẻ quốc tế cho khách hàng toàn cầu.</p>
          </div>
        </div>
      </section>
