...
| TASK-010 | Thanh toán Sandbox + phát hành Voucher_code (**lõi hệ thống**) | FR-08, R19.2, R24.3           | TASK-009                 |
...
### Trạng thái TV3 cập nhật ngày 2026-08-25

| TASK     | Trạng thái             | Bàn giao hiện có                                                                                                       |
| -------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| TASK-006 | **Hoàn thành**         | Đăng ký/hồ sơ đối tác, Admin duyệt hoặc khóa, CRUD chi nhánh, đóng/mở chi nhánh bằng `isActive`, RBAC và ownership.    |
| TASK-007 | **Hoàn thành**         | CRUD voucher, tải ảnh, chọn chi nhánh, gửi duyệt, Admin duyệt/công bố, pause/resume/discontinue và thống kê mã.        |
| TASK-013 | **Hoàn thành**         | Kiểm tra mã, quét QR bằng camera, xác nhận một/nhiều lượt, giới hạn theo đối tác/chi nhánh/nhân viên và ghi usage log. |
| TASK-014 | **Hoàn thành mở rộng** | Review 1–5 sao sau đơn `PAID`, danh sách/thống kê, sửa và xóa; đã tích hợp backend, frontend và migration.             |
| TASK-021 | **Hoàn thành**         | Workspace Đối tác/Nhân viên: tổng quan, hồ sơ, chi nhánh, voucher, nhân viên, redeem và báo cáo.                       |

E2E hiện có cho toàn bộ FLOW của TV3: `task006-partner-onboarding.spec.ts` (FLOW-005), `task007-voucher-product.spec.ts` (FLOW-006) và `task021-partner-workspace.spec.ts` (FLOW-007, FLOW-008).
...
# Kế hoạch MVP 2 đợt ban đầu và trạng thái mở rộng hiện tại
...
>
> Kế hoạch ban đầu chốt 13 bảng để chạy đường tiền. Sau đó nhóm đã mở rộng và schema hiện có **19 model**. Các phần từng đánh dấu OUT gồm `reviews`, `content_items`, `audit_logs`, `partner_staff` và `partner_staff_branches` đều đã được triển khai.
>
> Vì vậy TASK-014 (Review), TASK-017 (Content) và TASK-018 (Audit) không còn là “bỏ khỏi hệ thống”; đây là các chức năng mở rộng đã có code/database. Phạm vi demo tối thiểu vẫn có thể ưu tiên các FLOW mức Cao.
>
> **Một bảng `users` duy nhất cho mọi login** (khách / chủ đối tác / nhân viên / admin). `partners` là **hồ sơ doanh nghiệp** (không có mật khẩu), `partners.owner_user_id → users`. Mỗi nhân viên dùng một dòng `users` và một dòng `partner_staff`; phạm vi chi nhánh nằm trong `partner_staff_branches`.
...
## Phân công code theo domain — cập nhật theo phạm vi đã triển khai
...
| **TV3**       | Đối tác & Voucher (BE+FE)                     | 006, 007, 013, 014, 021          | 004, 005, 006, 007, 008     | ~10.5 |
| **TV4**       | Admin + Seed (BE+FE) +**dẫn integration/e2e** | 015, 017, 018, 019, 022, **023** | 010, 011, 012 + integration | ~10.5 |
...
- **TV2** — search/filter + chi tiết (008); cart + order (009); **thanh toán Sandbox + phát hành mã nguyên tử (010 — lõi P1–P5/P21)**. Gánh phần rủi ro nhất → không thêm FE để dồn sức cho transaction.
- **TV3** — partner + admin duyệt (006); voucher product trọn máy trạng thái (007); redeem (013 — P8–P11 → KPI-03); review (014); FE đối tác/nhân viên (021). Các hạng mục này hiện đã hoàn thành.
- **TV4** — admin quản đơn + hủy/hoàn (015 — P6/P15); content (017); audit (018); seed (019); FE admin (022); dẫn integration cuối (023). Content và audit từng nằm ngoài MVP tối thiểu nhưng hiện đã được triển khai.
...
| FLOW-004 | Đánh giá voucher sau khi mua      | TV3         | 014             |
...
| FLOW-008 | Báo cáo đối tác                   | TV3 + chung | 021 + 016       |
| FLOW-009 | Admin quản lý/phân quyền user     | TV1         | 004             |
| FLOW-010 | Hủy / hoàn đơn                    | TV4         | 015             |
| FLOW-011 | Nội dung và dashboard quản trị    | TV4 + chung | 017 + 022 + 016 |
| FLOW-012 | Ghi và tra cứu audit log          | TV4         | 018 + 022       |
...
> **Mục tiêu**: chạy thông `đăng ký → đối tác tạo voucher → admin duyệt → khách tìm → thêm giỏ → đặt đơn → thanh toán → nhận mã`. Bản hiện tại đã mở rộng từ Payment Sim sang VNPay, PayPal, OnePay và Stripe Sandbox.
>
...
## ĐỢT 2 ban đầu — Nối +4 bảng để đạt 13 bảng ưu tiên Cao
...
>
> **+4 bảng** (mỗi cái chỉ _trỏ vào_ core, một `prisma migrate` mới, KHÔNG sửa schema cũ): `categories`, `branches`, `voucher_product_branches`, `usage_logs`.
>
> Sau đợt này, các migration mở rộng tiếp tục bổ sung payment transaction, nhân viên, review, content và audit; xem schema as-built tại `docs/05-database-design/README.md`.
