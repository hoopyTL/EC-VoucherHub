...
async function main() {
  ...
  for (let index = 0; index < 42; index += 1) {
    ...
    const status =
      index % 9 === 0 ? OrderStatus.CANCELLED : index % 11 === 0 ? OrderStatus.PENDING_PAYMENT : OrderStatus.PAID
    ...
    await prisma.$transaction(async (tx) => {
      const order = await tx.order.create({
        data: {
          ...
          totalAmount,
          paymentMethod: index % 2 ? 'VNPAY' : 'STRIPE',
          status,
          paidAt: status === OrderStatus.PAID ? createdAt : null,
          createdAt,
          giftRecipient: { analyticsDemo: true, campaign: 'business-dashboard-2026' },
      ...
      if (status === OrderStatus.PAID) {
        ...
        await tx.issuedVoucherCode.create({
          data: {
            code: `VHDEMO${String(index + 1).padStart(6, '0')}`,
            orderId: order.id,
