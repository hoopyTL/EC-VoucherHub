...
async function main() {
  ...
  for (const [email, legalName, taxCode, representative] of catalogue) {
    ...
    const partner = byOwner
      ? await prisma.partner.update({ where: { id: byOwner.id }, data: { legalName, representative, approvalStatus: 'APPROVED', operatingStatus: 'ACTIVE' } })
      : await prisma.partner.create({ data: { ownerUserId: user.id, legalName, taxCode, representative, approvalStatus: 'APPROVED', operatingStatus: 'ACTIVE' } })
  ...
  await prisma.user.updateMany({ where: { role: { name: { in: ['PARTNER', 'DOI_TAC'] } } }, data: { passwordHash, status: 'ACTIVE', roleId: role.id } })
  await prisma.partner.updateMany({ data: { approvalStatus: 'APPROVED', operatingStatus: 'ACTIVE', rejectReason: null } })

  const partners = await prisma.partner.findMany({ include: { branches: true, owner: true }, orderBy: { legalName: 'asc' } })
  for (const partner of partners) {
    for (const [suffix, address, region] of branchSeeds) {
      if (!partner.branches.some((branch) => branch.region === region)) {
        await prisma.branch.create({ data: { partnerId: partner.id, name: `${partner.legalName} · ${suffix}`, address, region } })
      }
    }
  }
  ...
  for (const voucher of vouchers) {
    ...
    await prisma.$transaction([
      prisma.voucherProduct.update({ where: { id: voucher.id }, data: { partnerId: partner.id, description: `${voucher.name} do ${partner.legalName} cung cấp. Áp dụng tại hệ thống chi nhánh tham gia trên toàn quốc.` } }),
      prisma.voucherProductBranch.deleteMany({ where: { voucherProductId: voucher.id } }),
      prisma.voucherProductBranch.createMany({ data: partner.branches.map((branch) => ({ voucherProductId: voucher.id, branchId: branch.id })), skipDuplicates: true })
    ])
  }
  ...
  console.table(summary.map((partner) => ({ partner: partner.legalName, email: partner.owner.email, branches: partner._count.branches, vouchers: partner._count.voucherProducts })))
}
