...
const voucherNames = [
  'Buffet hải sản tối dành cho hai người',
  'Liệu trình massage thư giãn 90 phút',
  'Combo vé phim và bắp nước cuối tuần',
  'Tour nghỉ dưỡng biển 3 ngày 2 đêm',
  'Set lẩu gia đình bốn người',
  'Gói tập gym và yoga một tháng',
  'Chăm sóc da chuyên sâu chuẩn Hàn',
  'Phiếu mua sắm hàng tiêu dùng 500K',
  'Khóa học kỹ năng số thực chiến',
  'Combo cà phê rang xay và bánh ngọt',
  'Vé khu vui chơi trọn ngày cho bé',
  'Voucher nội thất cho căn hộ mới',
  'Brunch cuối tuần phong cách Địa Trung Hải',
  'Gói nghỉ dưỡng chăm sóc sức khỏe',
  'Vé xem phim IMAX dành cho hai người',
  'Tour khám phá miền Tây một ngày',
  'Tiệc sinh nhật tại nhà hàng',
  'Gói huấn luyện cá nhân 10 buổi',
  'Trang điểm dự tiệc cao cấp',
  'Giỏ quà thực phẩm hữu cơ',
  'Khóa tiếng Anh giao tiếp 12 buổi',
  'Thẻ đồ uống mùa hè 10 món',
  'Workshop sáng tạo cuối tuần cho bé',
  'Gói thiết kế góc làm việc tại nhà'
] as const
...
async function main() {
  ...
  for (let index = 0; index < businesses.length; index += 1) {
    ...
    const owner = await prisma.user.upsert({
      ...
      create: {
        ...
        phone: `0908${String(410000 + index).slice(-6)}`,
        ...
        data: [
          {
            ...
            name: `${legalName} - Chi nhánh trung tâm`,
            address: `${40 + index} Nguyễn Huệ, Quận 1, TP. Hồ Chí Minh`,
            region: 'TP. Hồ Chí Minh'
          },
          {
            partnerId: partner.id,
            name: `${legalName} - Chi nhánh phía Đông`,
            address: `${90 + index} Võ Nguyên Giáp, TP. Thủ Đức`,
            region: 'TP. Hồ Chí Minh'
          }
        ]
      })
  ...
  for (let index = 0; index < voucherNames.length; index += 1) {
    ...
    saleEnd.setDate(saleEnd.getDate() + 75)
    const usageEnd = new Date(now)
    usageEnd.setDate(usageEnd.getDate() + 120)
    ...
    const voucher =
      ...
      (await prisma.voucherProduct.create({
        data: {
          ...
          name: voucherNames[index],
          description: `${marker} Ưu đãi mới đang chờ đội ngũ VoucherHub kiểm duyệt nội dung, điều kiện và thời hạn sử dụng.`,
          ...
          saleStart,
          saleEnd,
          usageStart: saleStart,
          usageEnd,
          totalQuantity: 300 + index * 20,
          remainingQuantity: 300 + index * 20,
          status: VoucherStatus.PENDING_REVIEW,
          voucherProductBranches: partner.branches.length
            ? { create: partner.branches.slice(0, 2).map((branch) => ({ branchId: branch.id })) }
            : undefined
        }
      }))
    if (existing && voucher.status !== VoucherStatus.PENDING_REVIEW) {
      await prisma.voucherProduct.update({
        where: { id: voucher.id },
        data: { status: VoucherStatus.PENDING_REVIEW, rejectReason: null }
      })
    }
  }

  const existingOrder = await prisma.order.findFirst({
    where: { giftRecipient: { path: ['adminOperationsDemo'], equals: true } }
  })
  if (!existingOrder) {
    for (let index = 0; index < 84; index += 1) {
      ...
      createdAt.setHours(8 + (index % 12), index % 60)
      const status =
        index % 10 === 0
      ...
      await prisma.order.create({
        data: {
          ...
          totalAmount: Number(voucher.salePrice) * quantity,
          paymentMethod: index % 3 === 0 ? 'STRIPE' : 'VNPAY',
          status,
          paidAt: status === OrderStatus.PAID || status === OrderStatus.REFUNDED ? createdAt : null,
          createdAt,
          giftRecipient: { adminOperationsDemo: true, reference: `VH-OPS-${String(index + 1).padStart(4, '0')}` },
  ...
  console.log(
    `Đã sẵn sàng: ${pendingPartners.length} đối tác chờ duyệt, ${voucherNames.length} voucher chờ duyệt và 84 đơn vận hành.`
  )
}
